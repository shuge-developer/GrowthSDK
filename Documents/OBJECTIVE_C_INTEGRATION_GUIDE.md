# GrowthKit SDK Objective-C 集成指南

## 🎯 Objective-C 支持

GrowthKit SDK 现在完全支持 Objective-C 项目，提供了与 Swift 相同的功能。

## 🚀 快速集成

### 1. 导入头文件

在你的 Objective-C 文件中导入 GrowthKit：
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [self initializeGrowthKitSDK];
    
    return YES;
}

- (void)initializeGrowthKitSDK {
    NSLog(@"[AppDelegate] 🚀 开始初始化GrowthKit SDK");
    ...
    [[GameWebWrapper shared] initializeWithConfig:config completion:^(BOOL success, NSString * _Nullable errorMessage) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                NSLog(@"[AppDelegate] ✅ GrowthKit SDK初始化成功");
            } else {
                NSLog(@"[AppDelegate] ❌ GrowthKit SDK初始化失败: %@", errorMessage);
            }
        });
    }];
}
...
- (void)scene:(UIScene *)scene willConnectToSession:(UISceneSession *)session options:(UISceneConnectionOptions *)connectionOptions {
    if ([scene isKindOfClass:[UIWindowScene class]]) {
        ...
        [self initializeUnityAndGrowthKit];
    }
}

- (void)initializeUnityAndGrowthKit {
    NSLog(@"[SceneDelegate] 🎮 开始初始化Unity和GrowthKit集成");
...
- (void)setupGrowthKitBridge:(UIViewController *)unityController {
    NSLog(@"[SceneDelegate] 🔗 设置GrowthKit桥接器");
    
    // 创建GrowthKit桥接器作为根控制器
    GrowthKitUIKitBridge *growthKitBridge = [[GrowthKitUIKitBridge alloc] initWithUnityController:unityController];
    
    // 设置为根控制器
    self.window.rootViewController = growthKitBridge;
    [self.window makeKeyAndVisible];
    
    NSLog(@"[SceneDelegate] ✅ Unity + GrowthKit集成完成");
}
...
### GrowthKitNetworkConfig

```objc
@interface GrowthKitNetworkConfig : NSObject
...
- (void)initializeWithConfig:(GrowthKitNetworkConfig *)config 
                  completion:(void(^)(BOOL success, NSString * _Nullable errorMessage))completion;

@end
```

### GrowthKitUIKitBridge

```objc
@interface GrowthKitUIKitBridge : UIViewController
...
GrowthKit SDK需要正确配置才能支持Objective-C调用。使用提供的构建脚本：
...
3. **SWIFT_OBJC_INTERFACE_HEADER_NAME = "GrowthKit-Swift.h"** - 指定桥接头文件名
...
- `GrowthKit.h` - Umbrella header
- `GrowthKit-Swift.h` - 自动生成的Swift桥接头文件
...
1. **导入头文件** - 确保正确导入 `<GrowthKit/GrowthKit-Swift.h>` 或 `"GrowthKit-Swift.h"`
...
1. **'GrowthKit-Swift.h' file not found**
