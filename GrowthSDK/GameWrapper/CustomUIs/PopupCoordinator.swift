...
final class PopupCoordinator: ObservableObject {
    ...
    func updatePopupPosition(for area: AdArea) {
        ...
        print("[PopupCoordinator] 📍 更新弹窗位置: position=\(position), centerPoint=\(centerPoint)")
    }
    ...
    private func determinePopupPosition(for area: AdArea) -> Position {
        ...
        if adHeight < anchorHeightThreshold {
            ...
            if adCenterY < screenHeight * 0.5 {
                // 广告在屏幕上半部分，认为是顶部锚定
                print("[PopupCoordinator] 📌 检测到顶部锚定广告（高度=\(adHeight)，中心Y=\(adCenterY)），使用顶部弹窗")
                return .top
            } else {
                // 广告在屏幕下半部分，认为是底部锚定
                print("[PopupCoordinator] 📌 检测到底部锚定广告（高度=\(adHeight)，中心Y=\(adCenterY)），使用底部弹窗")
                return .bottom
            }
        }
        ...
        if adCenterY < topThreshold {
            // 广告在屏幕顶部
            print("[PopupCoordinator] 📌 广告在顶部区域（高度=\(adHeight)，中心Y=\(adCenterY)），使用顶部弹窗")
            return .top
        } else if adCenterY > bottomThreshold {
            // 广告在屏幕底部
            print("[PopupCoordinator] 📌 广告在底部区域（高度=\(adHeight)，中心Y=\(adCenterY)），使用底部弹窗")
            return .bottom
        } else {
            // 广告在屏幕中间
            print("[PopupCoordinator] 📌 广告在中间区域（高度=\(adHeight)，中心Y=\(adCenterY)），使用中心弹窗")
            return .center
        }
    }
    ...
    private func calculatePopupCenter(for area: AdArea, position: Position) -> CGPoint {
        ...
        // 打印调试信息
        print("[PopupCoordinator] 📊 广告区域: left=\(area.left), top=\(area.top), width=\(area.width), height=\(area.height)")
        print("[PopupCoordinator] 📊 广告中心点: \(area.center)")
        print("[PopupCoordinator] 📊 调整后的广告中心点Y: \(area.getAdjustedCenterY())")
        ...
        case .top:
            ...
            print("[PopupCoordinator] 📐 顶部弹窗使用默认位置: Y=\(topY)")
            return CGPoint(x: screenWidth/2, y: topY)
            
        case .center:
            // 中心弹窗：直接使用广告区域的中心点
            let adCenterY = area.getAdjustedCenterY()
            print("[PopupCoordinator] 📐 中心弹窗Y位置: \(adCenterY)")
        ...
        case .bottom:
            ...
            print("[PopupCoordinator] 📐 底部弹窗使用默认位置: Y=\(bottomY)")
