...
/// Used for parsing ASN.1 encoded data structures in RSA keys
internal class ASN1Scanner {
...
internal extension Data {
    ...
    func stripKeyHeader() throws -> Data {
        ...
        guard case .sequence(let nodes) = node else {
            throw ParserError.invalidAsn1RootNode
        }
        ...
        throw ParserError.invalidAsn1Structure
    }
}
...
internal enum Asn1Parser {
    ...
    static func parse(data: Data) throws -> Node {
        let scanner = ASN1Scanner(data: data)
    ...
    /// - Returns: Parsed node
    /// - Throws: A ParserError if anything goes wrong, or if an unknown node was encountered
    private static func parseNode(scanner: ASN1Scanner) throws -> Node {
