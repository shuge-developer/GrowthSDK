...
internal struct AdThinking {
    ...
    static func adLoadFail(_ style: AdStyle? = nil, error: AdError) {
        ...
        ThinkListener.log(.sdk_adFail) {
            params
        }
    }
    
    static func adShowFail(_ error: AdError) {
        ThinkListener.log(.sdk_adShowFail) {
            [.errmsg: error.errMsg]
        }
    }
