...
internal final class GrowthKitLayerManager: ObservableObject {
    ...
    func adjustUnityLayer(hostController: UIViewController?, unityView: UIView?) {
        guard let unityView = unityView, let hostController = hostController else {
            print("[GrowthKit] ⚠️ Unity视图或宿主控制器不可用，跳过原生层级调整")
            return
        }
        let bringToFront = topLayerType == .unity
        if bringToFront {
            hostController.view.bringSubviewToFront(unityView)
            print("[GrowthKit] 📤 Unity视图已移至前台")
        } else {
            hostController.view.sendSubviewToBack(unityView)
            print("[GrowthKit] 📥 Unity视图已移至后台")
        }
        print("[GrowthKit] 🔄 原生视图层级已调整: Unity \(bringToFront ? "在顶层" : "在底层")")
    }
    ...
    private func updateTopLayerType() {
        ...
        if newTopLayer != topLayerType {
            ...
            print("[GrowthKit] 🔝 顶层切换为: \(topLayerType.displayName)")
            print("[GrowthKit] 🔢 Unity zIndex: \(unityZIndex), WebView zIndex: \(sWebZIndex)")
        }
    }
}
